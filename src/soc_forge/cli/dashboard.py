...
class ThreatIntelligenceDashboard:
    ...
    def display_threat_intelligence_dashboard(self, ip: str, analysis_results: Dict[str, Any]) -> None:
        ...
        # Display DNSBL results
        self._display_dnsbl_results(analysis_results)
...
"""
        ...
        # Get threat score for visual indicator
        threat_score = ThreatScorer.calculate_overall_threat_score(analysis_results)
        ...
        # Create info panel
        info_text = f"""
[bold white]Target IP:[/bold white] [cyan]{ip}[/cyan]
[bold white]Scan Time:[/bold white] {current_time}
[bold white]Threat Level:[/bold white] [{threat_color}]{threat_level}[/{threat_color}] [{threat_color}][{threat_bar}][/{threat_color}] [{threat_color}]{threat_score}/100[/{threat_color}]
[bold white]Sources:[/bold white] VirusTotal • AbuseIPDB • Shodan • AlienVault OTX • GreyNoise • IPInfo
"""
    ...
    def _display_geographic_network_info(self, analysis_results: Dict[str, Any]) -> None:
        ...
        self.console.print("\n")
        header = Panel(
            "[bold white]🌍 Geographic & Network Information[/bold white]",
            style="bold cyan",
            box=box.DOUBLE
        )
        self.console.print(header)
    ...
    def _display_open_ports_services(self, analysis_results: Dict[str, Any]) -> None:
        ...
        self.console.print("\n")
        header = Panel(
            "[bold white]🔍 Open Ports & Services (Shodan)[/bold white]",
            style="bold magenta",
            box=box.DOUBLE
        )
        self.console.print(header)
    ...
    def _display_virustotal_vendor_analysis(self, analysis_results: Dict[str, Any]) -> None:
        ...
        self.console.print("\n")
        header = Panel(
            "[bold white]🛡️ VirusTotal - Key Vendor Analysis[/bold white]",
            style="bold red",
            box=box.DOUBLE
        )
        self.console.print(header)
    ...
    def _display_otx_threat_pulses(self, analysis_results: Dict[str, Any]) -> None:
        ...
        self.console.print("\n")
        header = Panel(
            "[bold white]👁️ AlienVault OTX - Threat Intelligence[/bold white]",
            style="bold yellow",
            box=box.DOUBLE
        )
        self.console.print(header)
    ...
    def _display_threat_feeds_results(self, analysis_results: Dict[str, Any]) -> None:
        """Display threat intelligence feeds results"""
        self.console.print("\n")
        header = Panel(
            "[bold white]📋 Threat Intelligence Feeds (100+ Sources)[/bold white]",
            style="bold red",
            box=box.DOUBLE
        )
        self.console.print(header)

        feed_data = analysis_results.get('threat_feeds', {})

        if not feed_data or feed_data.get('error'):
            self.console.print("[dim]Threat feeds not available or error occurred[/dim]")
            return

        if not feed_data.get('found'):
            self.console.print("[green]✓ IP not found in any threat intelligence feeds[/green]")
            self.console.print(f"[dim]Checked {feed_data.get('total_feeds_checked', 0)} threat feeds[/dim]")
            return

        # Display summary
        feed_count = feed_data.get('feed_count', 0)
        threat_level = feed_data.get('threat_level', 'UNKNOWN')
        ...
        summary = f"""
[bold red]⚠ IP FOUND IN THREAT FEEDS ⚠[/bold red]

[bold white]Threat Level:[/bold white] [{level_color}]{threat_level}[/{level_color}]
[bold white]Feed Matches:[/bold white] {feed_count} out of {feed_data.get('total_feeds_checked', 0)} feeds
[bold white]Threat Categories:[/bold white] {', '.join(categories) if categories else 'Unknown'}
"""
        self.console.print(summary)

        # Display individual feed matches
        feeds = feed_data.get('feeds', [])
        if feeds:
            self.console.print("\n[bold]Matched Feeds:[/bold]")

            # Group feeds by category
            from collections import defaultdict
            by_category = defaultdict(list)
            for feed in feeds:
                # Extract category from feed name (simplified)
                if 'abuse.ch' in feed.lower():
                    by_category['Malware/Abuse'].append(feed)
                elif 'blocklist' in feed.lower():
                    by_category['Blocklists'].append(feed)
                elif 'ipsum' in feed.lower():
                    by_category['IPsum Threat Levels'].append(feed)
                elif 'c2' in feed.lower() or 'cobalt' in feed.lower() or 'metasploit' in feed.lower():
                    by_category['C2/Command & Control'].append(feed)
                elif 'phishing' in feed.lower():
                    by_category['Phishing'].append(feed)
                elif 'spam' in feed.lower():
                    by_category['Spam/Mail'].append(feed)
                else:
                    by_category['Other'].append(feed)

            # Display by category
            for category, category_feeds in by_category.items():
                self.console.print(f"\n  [bold cyan]{category}:[/bold cyan]")
                for feed in category_feeds[:10]:  # Limit to 10 per category
                    self.console.print(f"    • {feed}")
                if len(category_feeds) > 10:
                    self.console.print(f"    [dim]... and {len(category_feeds) - 10} more[/dim]")

    def _display_dnsbl_results(self, analysis_results: Dict[str, Any]) -> None:
        """Display DNSBL (DNS Blacklist) results"""
        self.console.print("\n")
        header = Panel(
            "[bold white]🔍 DNS Blacklist Check (50+ DNSBL Servers)[/bold white]",
            style="bold magenta",
            box=box.DOUBLE
        )
        self.console.print(header)

        dnsbl_data = analysis_results.get('dnsbl', {})

        if not dnsbl_data or dnsbl_data.get('error'):
            self.console.print("[dim]DNSBL check not available or error occurred[/dim]")
            return

        listing_count = dnsbl_data.get('listing_count', 0)
        threat_level = dnsbl_data.get('threat_level', 'UNKNOWN')
        is_whitelisted = dnsbl_data.get('is_whitelisted', False)
        total_checked = dnsbl_data.get('total_checked', 0)
        listings = dnsbl_data.get('listings', dnsbl_data.get('listed_in', []))
        ...
        blacklist_count = dnsbl_data.get('blacklist_count', len(blacklist_entries))
        ...
        # Show clean status only if not in any lists
        if listing_count == 0:
            self.console.print("[green]✓ IP not listed in any DNS blacklists[/green]")
            self.console.print(f"[dim]Checked {total_checked} DNSBL servers[/dim]")
            return
        ...
        categories = dnsbl_data.get('categories', [])
...
"""
        ...
        # Display individual DNSBL listings
        if blacklist_entries:
            self.console.print("\n[bold]Listed in DNSBLs:[/bold]")

            # Create table for listings
            dnsbl_table = Table(box=box.SIMPLE, padding=(0, 1), show_header=True)
            dnsbl_table.add_column("DNSBL", style="bold white", width=30)
            dnsbl_table.add_column("Category", width=15)
            dnsbl_table.add_column("Result", width=20)

            for listing in blacklist_entries[:20]:  # Show first 20 listings
                ...
                dnsbl_table.add_row(name, category_display, result)

            self.console.print(dnsbl_table)

            if len(blacklist_entries) > 20:
                remaining = len(blacklist_entries) - 20
                self.console.print(f"\n[dim]... and {remaining} more DNSBL listings[/dim]")

    def _display_recent_report_tags(self, analysis_results: Dict[str, Any]) -> None:
        """Display recent report tags from all sources"""
        self.console.print("\n")
        header = Panel(
            "[bold white]🏷️ Aggregated Threat Tags[/bold white]",
            style="bold green",
            box=box.DOUBLE
        )
        self.console.print(header)
    ...
    def _display_references_urls(self, ip: str) -> None:
        ...
        self.console.print("\n")
        header = Panel(
            "[bold white]🔗 References / URL Checkups[/bold white]",
            style="bold blue",
            box=box.DOUBLE
        )
        self.console.print(header)
