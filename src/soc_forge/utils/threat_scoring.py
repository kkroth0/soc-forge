...
class ThreatScorer:
    ...
    @staticmethod
    def calculate_overall_threat_score(analysis_results: Dict[str, Any]) -> int:
        """
        Calculate overall threat score including feeds and DNSBLs
        ...
        """
        # Start with base IP threat score from APIs
        score = ThreatScorer.calculate_ip_threat_score(analysis_results)

        # Add threat feeds scoring (max 20 points)
        if 'threat_feeds' in analysis_results and analysis_results['threat_feeds'].get('found'):
            feeds = analysis_results['threat_feeds']
            feed_count = feeds.get('feed_count', 0)
            threat_level = feeds.get('threat_level', 'CLEAN')
            ...
            if threat_level == 'CRITICAL':
                ...
                # Add based on feed count
                score += min(10, feed_count)

        # Add DNSBL scoring (max 15 points)
        if 'dnsbl' in analysis_results and analysis_results['dnsbl'].get('found'):
            dnsbl = analysis_results['dnsbl']
            listing_count = dnsbl.get('listing_count', 0)
            threat_level = dnsbl.get('threat_level', 'CLEAN')
            ...
            if threat_level == 'CRITICAL':
                ...
                # Add based on listing count
                score += min(10, listing_count // 2)
